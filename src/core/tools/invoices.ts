...
export function registerInvoiceTools(server: FastMCP) {
  server.addTool({
    name: "list_invoices",
    description: "List invoices in Papierkram. Returns compact summaries by default (set compact=false for full details). Supports pagination and filtering.",
    parameters: z.object({
    ...
    execute: async (params) => {
      ...
      const result = await client.list("/income/invoices", query as Record<string, string | number | boolean>);
      if (compact === false) return JSON.stringify(result, null, 2);
      return JSON.stringify(compactList(result as Record<string, unknown>, compactInvoice), null, 2);
    },
  });
