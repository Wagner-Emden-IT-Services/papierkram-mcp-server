...
> **Canonical agent instructions — single source of truth for every coding agent.**
> Codex CLI reads this file natively. Claude Code loads it via `@AGENTS.md` in `CLAUDE.md`.
> Keep all shared project rules here; do **not** duplicate them into `CLAUDE.md`.
...
This repo carries local Claude Code skills under `.claude/skills/` (git-ignored, machine-local — not shipped with the repo). Repo-relevant ones:

- **`test-runner`** — run and manage the test suite (all tools, resources, prompts). Use it for step 2 of the Pre-Push Checklist above.
- **`mcp-builder`** — reference for building/extending MCP servers when adding or refactoring tools.

Claude Code auto-discovers and can invoke these (`/test-runner`). Codex has no skill auto-loading — treat them as pointers and run the equivalent commands directly (`npm test`, etc.).
